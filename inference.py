...
def main():
    ...
    # Resolve torch device from args.gpu (int from YAML/CLI).
    # Convention: gpu < 0 => CPU; gpu >= 0 => CUDA device index.
    if getattr(args, 'gpu', None) is None or int(args.gpu) < 0 or not torch.cuda.is_available():
    ...
    for i, batch_data in pbar:
        ...
        image = image.as_tensor().to(device, non_blocking=use_cuda)
        # If input spatial size doesn't match model expected `input_size`, resample here.
        # Model `input_size` is (H, W, D). Data tensor shape expected: (B, C, H, W, D).
        try:
            target_size = getattr(model, 'input_size', None)
            if target_size is not None and image.dim() == 5:
                B, C, H, W, D = image.shape
                tgt_h, tgt_w, tgt_d = target_size
                if (H, W, D) != (tgt_h, tgt_w, tgt_d):
                    # interpolate expects (N, C, D, H, W) with size=(D_out, H_out, W_out)
                    image = image.permute(0, 1, 4, 2, 3)
                    size = (int(tgt_d), int(tgt_h), int(tgt_w))
                    image = torch.nn.functional.interpolate(image, size=size, mode='trilinear', align_corners=False)
                    image = image.permute(0, 1, 3, 4, 2)
        except Exception as e:
            print('Warning: resize failed, proceeding without resize:', e)
        clinical_variables = {}
        for k, v in batch_data.items():
            if k in clinical_variable_list:
                if isinstance(v, torch.Tensor):
                    clinical_variables[k] = v.to(device, non_blocking=use_cuda)
                else:
                    clinical_variables[k] = v

        event = event.to(device, non_blocking=use_cuda)
        duration = duration.to(device, non_blocking=use_cuda)
