...
def preprocessing_mri(patient_dir):
    ...
    # mri 分区提取
    img = nifti_t1_bet.numpy()
    ce_mask = CE_tumor_mask.numpy()
    ne_mask = NE_tumor_mask.numpy()
    wt_mask = WT_tumor_mask.numpy()

    # 一区：强化肿瘤 (CE)
    region1 = nifti_t1_bet.new_image_like(img * ce_mask)
    # 二区：坏死水肿 (NE)
    region2 = nifti_t1_bet.new_image_like(img * ne_mask)
    # 三区：全肿瘤 (WT)
    region3 = nifti_t1_bet.new_image_like(img * wt_mask)

    # 保存三个分区（模型后续读取这三个文件）
    region1.to_file(os.path.join(target_dir, 'region1_CE.nii.gz'))
    region2.to_file(os.path.join(target_dir, 'region2_NE.nii.gz'))
    region3.to_file(os.path.join(target_dir, 'region3_WT.nii.gz'))
    
    # remove bet files
    for filename in ['t1_bet.nii.gz', 't1ce_bet.nii.gz', 't2_bet.nii.gz', 'flair_bet.nii.gz']:
